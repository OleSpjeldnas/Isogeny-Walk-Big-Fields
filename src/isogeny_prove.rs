...
// Commit to a randomized vector based on polynomial
fn commit_with_rand(f: DensePolynomial<F>, eval_domain: Vec<F>) -> (Vec<F>, FieldMT, Vec<F>) {
    ...
    let randomized_evals = vec![
        evals_slice,
        //vec![vec![Fp::from(0),Fp::from(0)]; 2u64.pow(k) as usize - s_ord as usize],
        random_vec_slice,
        //vec![vec![Fp::from(0),Fp::from(0)]; 2u64.pow(k) as usize - s_ord as usize],
    ]
    .concat();
...
// Function to compute pseudorandom list of n indices in span [0, 2^k] given some input c, i.e. first
...
fn blinded_eval_first_six_steps(
    ...
    let k: u32 = (((s_ord as f32).log2()).ceil()) as u32;
    //println!("s_ord: {:?}, 2^k: {:?}", s_ord, 2u64.pow(k));
    // Randomly commit to g
...
//the evaluation domain is E = r . Finally, s_ord is the size of E.   ->    (Challenges, roots, roots_fri, paths_fri, paths, points_fri, points)
pub fn prove(
