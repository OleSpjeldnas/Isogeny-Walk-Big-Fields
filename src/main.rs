...
fn lines_from_file(filename: impl AsRef<Path>) -> io::Result<Vec<F>> {
    ...
    for line in lines {
        ...
        if !line.contains("*a") {
            ...
            b = Fp::from_str(parts.next().unwrap().trim()).map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "Failed to parse Fp2"))?;
            a = Fp::from(0);
        } else {
            let mut parts = line.trim().split("*a +");
            b = Fp::from_str(parts.next().unwrap()).map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "Failed to parse Fp1"))?;
            a = Fp::from_str(parts.next().unwrap().trim()).map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "Failed to parse Fp5"))?;
        }
...
fn round(i: usize) -> Option<(u64, u64, f32)> {
    ...
    let witness: DensePolynomial<F> = DensePolynomial { coeffs: lines_from_file(&format!("Phis/polynomial_{}.txt", 9+i)).unwrap() };
    ...
    // psi
    let psi: DensePolynomial<F> = DensePolynomial { coeffs: lines_from_file(&format!("Psis/polynomial_{}.txt", 9+i)).unwrap() };
    ...
    if b {
        ...
        let proof_size = ((size1+size2+size3+size4+size5+size6+size7) as f32)/1024f32;
